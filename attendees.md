---
layout: page
title: Attendees
permalink: /attendees/
description: "Everything attendees need to know about Orlando Code Camp 2026 - registration, food, directions, and more."
nav_order: 3
---

## Save the Date!

Orlando Code Camp 2026 will be held at Seminole State College in Sanford, FL on **April 11, 2026** from **7:30 AM to 5:00 PM**. We're looking forward to seeing you! Here's some helpful information about the event:

{% include registration-banner.html %}

## Location

More information on the location of the event and parking can be found on the [Location page](/location/).

## Food

The following will be provided to attendees:

- **Breakfast** and coffee will be provided by Dunkin
- **Lunch** will be provided by Jason's Deli - it will include sandwiches, chips, and various drinks. *You'll choose your sandwich selection during Eventbrite registration.*
- An **afternoon snack**
- **Coffee, soda, and water** (all day!)

## After-Party

<div class="teaser-card">
  <div class="teaser-card-icon">🎉</div>
  <div class="teaser-card-content">
    <h3 class="teaser-card-title">Join Us After the Event!</h3>
    <p class="teaser-card-description">The learning doesn't stop when the sessions end. Join fellow attendees, speakers, and sponsors for food, drinks, and networking at our after-party.</p>
    <p class="teaser-card-note">Venue details coming soon — check back for updates!</p>
  </div>
</div>

## Raffle

Thanks to our excellent **[sponsors](/sponsors/)**, we'll be having a raffle for some fun prizes!

In order to participate:

1. Visit each of our sponsors for a stamp on your sponsors bingo card (provided on arrival at the event)
2. Drop off your bingo card at the ONETUG desk in the lobby (rotunda) at some point during the afternoon (make sure that your **name is written on the card**!) - you'll then receive a raffle ticket to hold onto until the raffle event
3. You **must be present** for the raffle give-away at the end of the event in order to win!
4. Only one (1) raffle ticket per attendee

<div class="banner banner-info mt-8">
  <strong>Pro tip:</strong> Arrive early to get the best parking spots and have time to grab breakfast before the first sessions begin!
</div>
